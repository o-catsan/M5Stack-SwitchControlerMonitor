...
unsigned long lastWifiSendMs = 0;
constexpr uint8_t BOOT_MODE_PIN = 4;
bool bootModeWiFi = false;
...
const unsigned long DEBUG_POLL_INTERVAL_MS = 1000;
...
void setup() {
    ...
    bootModeWiFi = (digitalRead(BOOT_MODE_PIN) == HIGH);
    ...
    M5.Display.println(bootModeWiFi ? "Boot mode: USB Host + WiFi" : "Boot mode: USB Host only");
    ...
    if (bootModeWiFi) {
        ...
        Serial.println("USB Host + WiFi mode ready");
    } else {
        WiFi.mode(WIFI_OFF);
        Serial.println("USB Host only mode ready");
    }
...
void drawControllerInfoTo(DisplayType& target, int16_t yOffset) {
    ...
    target.printf("ST:%02X INT:%d REV:%02X\n", usbTaskState, usbIntLevel,
                  max3421Revision);
    if (bootModeWiFi) {
        target.printf("WiFi:%s IP:%s\n", wifiClientConnected ? "connected" : "waiting",
                      WiFi.softAPIP().toString().c_str());
    } else {
        target.printf("WiFi:disabled GPIO4:OFF\n");
    }

    target.setTextColor(WHITE);
    target.setCursor(0, 62 - yOffset);
    if (padState.isPs5) {
...
void sendControllerState() {
    ...
    Serial2.print("\r\n");
    if (bootModeWiFi && wifiClientConnected && wifiClient.connected()) {
        wifiClient.write(reinterpret_cast<const uint8_t*>(lastTxData), strlen(lastTxData));
        wifiClient.write('\n');
    }
    if (ENABLE_VERBOSE_LOG) {
        Serial.print("BTN:");
...
void loop() {
    ...
    if (millis() - lastDebugPoll >= DEBUG_POLL_INTERVAL_MS) {
        ...
        Serial.printf("USB state=%02X INT=%d REV=%02X WiFi=%s\n",
                      usbTaskState, usbIntLevel, max3421Revision,
                      bootModeWiFi ? (wifiClientConnected ? "connected" : "waiting") : "disabled");
    }
