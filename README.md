...
`build.ps1` は `Documents/Arduino/libraries/USB_Host_Shield_Library_2.0` を事前チェックし、無ければインストールします。古いライブラリにのみ Core/Core2 のピン別名と、USB Host Shield Library 2.0 PR #843 相当のCoreS3定義をローカルへ冪等にパッチします。upstream に定義済みなら追加変更しません。
